...
Claude Code 的用量按**滚动 5 小时窗口**重置。窗口从你第一条消息开始计时，并**向下取整到整点**（如 10:20 发消息，窗口从 10:00 开始算）。

### 不做预热 vs 做预热

假设工作时间为 10:00–12:00、13:30–19:30（午休 12:00–13:30）：
...
不做预热：                    [=========== 窗口 1 ===========]
                               工作 10-12  午休  工作 13:30-15
                                                              [=========== 窗口 2 ===========]
                                                               工作 15-19:30 ⚠ 易在 18 点前打满
                                                                            ░░ 被限速但还没下班 ░░
...
> 预热后，下午工作被拆成两个窗口，各自额度充足，不再被限速卡住。

### 默认时间表

默认的 **07:01 / 12:01 / 17:01** 把窗口锚定到覆盖完整工作日的节奏：

```
窗口 1：07:01 → 12:01   ← 上班前预热，覆盖上午 10:00-12:00
窗口 2：12:01 → 17:01   ← 午休时触发，覆盖下午 13:30-17:00
窗口 3：17:01 → 22:01   ← 下午中段触发，覆盖收尾 17:00-19:30
```
...
```bash
git clone https://github.com/kaen98/claude-morning-cron.git
